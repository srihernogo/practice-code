...
public class Examples {
    ...
    void basicExample_ServiceThrowingException_v2() {
        ...
        SearchRequest request = new SearchRequest("NYC", "LAX", "07/31/2020");
        CheckedFunction0<List<Flight>> retryingFlightSearch = Retry.decorateCheckedSupplier(retry,
    ...
    void predicateExample() {
        RetryConfig config = RetryConfig.<SearchResponse>custom().maxAttempts(3).waitDuration(Duration.of(3, SECONDS))
                .retryOnResult(searchResponse -> searchResponse.getErrorCode().equals("FS-167")).build();
        RetryRegistry registry = RetryRegistry.of(config);
        Retry retry = registry.retry("flightSearchService", config);

        FlightSearchService service = new FlightSearchService();
        SearchRequest request = new SearchRequest("NYC", "LAX", "07/31/2020");
        CheckedFunction0<SearchResponse> retryingFlightSearch = Retry.decorateCheckedSupplier(retry,
    ...
    void retryExceptionsIgnoreExceptionsExample() {
        RetryConfig config = RetryConfig.custom().maxAttempts(3).waitDuration(Duration.of(3, SECONDS))
                .retryExceptions(FlightServiceBaseException.class).ignoreExceptions(SeatsUnavailableException.class)
                .build();
        RetryRegistry registry = RetryRegistry.of(config);
        ...
        CheckedFunction0<BookingResponse> bookingResponse2 = Retry.decorateCheckedSupplier(retry,
                bookingResponseSupplier2);
    ...
    void intervalFunction_Random() {
        RetryConfig config = RetryConfig.custom().maxAttempts(4).intervalFunction(IntervalFunction.ofRandomized(2000))
                .build();
        RetryRegistry registry = RetryRegistry.of(config);
        Retry retry = registry.retry("flightSearchService", config);
    ...
    void intervalFunction_Exponential() {
        RetryConfig config = RetryConfig.custom().maxAttempts(6)
                .intervalFunction(IntervalFunction.ofExponentialBackoff(1000, 2)).build();
        RetryRegistry registry = RetryRegistry.of(config);
        Retry retry = registry.retry("flightSearchService", config);
    ...
    void asyncRetryExample() {
        ...
        Supplier<CompletionStage<List<Flight>>> completionStageSupplier = () -> CompletableFuture
                .supplyAsync(() -> service.searchFlights(request));
    ...
    void retryOnException() {
        ...
        RetryConfig config = RetryConfig.custom().maxAttempts(3).waitDuration(Duration.of(1, SECONDS))
                .retryOnException(rateLimitPredicate).build();
        RetryRegistry registry = RetryRegistry.of(config);
        Retry retry = registry.retry("flightSearchService", config);
    ...
    void retryMetrics() {
        ...
        Consumer<Meter> meterConsumer = meter -> {
            ...
            Double metricValue = StreamSupport.stream(meter.measure().spliterator(), false)
                    .filter(m -> m.getStatistic().name().equals("COUNT")).findFirst().map(m -> m.getValue())
                    .orElse(0.0);
